...
from batch_generators import study_target_generator, hybrid_target_generator
...
class Trainer:
    ...
    def load_texts(self, inputs):
        ...
        for input in inputs:
            self.vecs[input] = pickle.load(open('../data/vectorizers/{}.p'.format(input), 'rb')) 
            if self.nb_train:
    ...
    def load_other_aspect_texts(self):
        self.other_vecs = {}
        all_targets = ['populations', 'interventions', 'outcomes']
        for t in all_targets:
            if t != self.target:
                self.other_vecs[t] = pickle.load(open('../data/vectorizers/{}.p'.format(t), 'rb'))
        
        # self.other_vecs[other_aspects[1]].X.shape
    ...
    def train(self, train_idxs, val_idxs, nb_epoch, batch_size, callback_list,
        ...
        # define callbacks
        cb = ModelCheckpoint(weight_str.format(self.target, fold, 'loss-min'),
                             monitor='loss',
                             save_best_only=True,
                             mode='min')
        ce = ModelCheckpoint(weight_str.format(self.target, fold, 'loss-every'),
        ...
        other_aspects = list(self.other_vecs.keys())

        #import pdb; pdb.set_trace()
        # why does self.other_vecs[other_aspects[0]].X.shape != self.other_vecs[other_aspects[1]].X.shape??!
        study_study_batch = hybrid_target_generator(X_study[val_idxs], X_target[val_idxs],
        ...
        # X_source, X_target, X_tilde_1, X_tilde_2, cdnos, exp_group, exp_id, nb_sample=128,
        ...
        self.model.fit_generator(gen_source_target_batches,
                                 samples_per_epoch=int((nb_train/batch_size))*batch_size,
                                 nb_epoch=nb_epoch,
                                 verbose=1,
